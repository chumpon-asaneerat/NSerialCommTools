...
namespace NLib.Data
{
        ...
        System.Data.OracleClient.OracleCommandBuilder>
    {
        ...
        protected override void InitFormatter(NDbFormatter formatter)
        {
            ...
            formatter.RequiredSPParameterPrefix = () =>
            {
                return false;
            };
        ...
        protected override bool CanCreateConnection()
        {
            ...
            if (!hasAllFiles)
            {
                "Cannot connect to oracle database because one or more instant client file(s) are missing.".Err();
                "Required file list below".Err();
                foreach (string fileName in requiredFiles)
                {
                    string msg = " - " + fileName;
                    msg.Err();
        ...
        protected override SchemaInfo<DbTable> GetTableSchemaInfo(string owner)
        {
            ...
            {                
                new DbRestriction(1, "Owner", _owner.ToUpper()),
                new DbRestriction(2, "Table", "")
            };
        ...
        {
            string _owner = string.IsNullOrWhiteSpace(owner) ? string.Empty : owner.Trim();
            DbRestriction[] restrictions = new DbRestriction[2];
            restrictions[0] = new DbRestriction(1, "OWNER", _owner);
            restrictions[1] = new DbRestriction(2, "NAME", string.Empty);
