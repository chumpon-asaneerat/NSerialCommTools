...
namespace NLib.Data
{
	...
	public class SqlServerBackupDatabaseTask : NDbTask
	{
		...
		public override NDbTaskResult Execute(NDbConnection manager, NDbTaskParameter parameter)
		{
			...
			try
			{
				"Grant permission for Backup File.".Info();
			...
			{
				"Grant Permission Failed.".Err(ex);
			}
			...
			try
			{
				if (null != fSecurity)
				{
					"Revoke permission for Backup File.".Info();
			...
			{
				"Revoke Permission Failed.".Err(ex);
			}
	...
	public class SqlServerRestoreDatabaseTask : NDbTask
	{
		...
		public override NDbTaskResult Execute(NDbConnection manager, NDbTaskParameter parameter)
		{
			...
			try
			{
				...
				"Grant permission for MDF File.".Info();
				// Get a FileSecurity object that represents the
				// current security settings.
				...
				"Grant permission for LDF File.".Info();
				// Get a FileSecurity object that represents the
				// current security settings.
			...
			try
			{
				...
				if (fSecurity2 != null)
				{
					"Ignore Revoke permission for MDF File.".Info();
					// Remove the FileSystemAccessRule from the security settings.
					/*
				...
				if (fSecurity3 != null)
				{
					"Ignore Revoke permission for LDF File.".Info();
					// Remove the FileSystemAccessRule from the security settings.
					/*
