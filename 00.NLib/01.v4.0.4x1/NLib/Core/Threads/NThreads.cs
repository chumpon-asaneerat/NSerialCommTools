...
namespace NLib.Threads
{
    ...
    public class NThread
    {
        ...
        private bool IsExit(NThreadState state)
        {
            ...
            {
                this.Info("Thread '{0}' is exit due to application is exit."
                    .args(this.Name));
                return true;
            }
            if (null == state)
            ...
            else
            {
                ...
                {
                    this.Info("Thread '{0}' is completed."
                        .args(this.Name));
                ...
                if (state.IsCanceled)
                {
                    this.Info("Thread '{0}' is cancel by user."
                        .args(this.Name));
                ...
                else
                {
                    ...
                    {
                        // Timeout is met
                        this.Info("Thread '{0}' is detected timeout."
                            .args(this.Name));
        ...
        private void CreateThread(int timeout)
        {
            if (_isRunning || null != _th)
            {
                ...
                this.Info("Thread '{0}' is already running. Kill exist thread"
                    .args(this.Name));
                KillThread();
            }
            ...
            if (null != _th)
            {
                ...
                this.Info("Thread '{0}' is still running. It's seem to be a problem."
                    .args(this.Name));
                return;
            }
            ...
            lock (this)
            {
                ...
                this.Info("Thread '{0}' is create and about to run."
                    .args(this.Name));
        ...
        private static void Remove(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                ...
                DebugManager.Instance.Info("Not allow thead without name.");
                return;
            }
            NThread th = null;
            ...
            if (_threads.ContainsKey(key))
            {
                lock (typeof(NThread))
                {
                    DebugManager.Instance.Info("Thread '{0}' is remove from list."
                        .args(name));
                    try
                    {
                        th = _threads[key];
        ...
        public static NThread Create(string name)
        {
            ...
            if (_threads.ContainsKey(key))
            {
                lock (typeof(NThread))
                {
                    ...
                    DebugManager.Instance.Info("Thread '{0}' already exists in list."
                        .args(name));
                }
            }
            else
            {
                lock (typeof(NThread))
                {
                    ...
                    DebugManager.Instance.Info("Thread '{0}' created and add to list."
                        .args(name));
                }
            }
            return th;
        ...
        public static void FreeThreads()
        {
            lock (typeof(NThread))
            {
                DebugManager.Instance.Info("Free all threads.");
